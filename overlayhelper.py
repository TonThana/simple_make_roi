...
def overlay_helper(roiData=None, imageData=None, title=None):
    ...
    # mask of nan value
    # nanMask = np.isnan(imageData)
    # roiMask = roiData == 1
    # roiNanOverlap = nanMask * roiMask
    ...
    for i in range(int(rowNum*colNum)):
        ...
        # ax.imshow(
        #     np.rot90(roiNanOverlap[:, :, i]), cmap=cm.get_cmap("viridis"))
